...
int main(){
    ...
    int pid1 = fork();
    if(pid1 == 0){
        printf(1, "Child process 1 started\n");
        printf(1, "Child process 1 going to sleep for 3 seconds..\n");
        sleep(3000);
        printf(1, "Child process 1 woke up. Exiting..\n");
        exit();
    }
    else{
        // Blocking call to reap the child process
        int status = waitpid(pid1, 1);
        if(status == pid1){
        ...
        if(pid2 == 0){
            sleep(100);
            printf(1, "Child process 2 started\n");
            printf(1, "Child process 2 going to sleep for 3 seconds..\n");
            sleep(3000);
            printf(1, "Child process 2 woke up. Exiting..\n");
            exit();
        }
        else{
            // Non-blocking call, expected to return immediately
            status = waitpid(pid2, 0);
            if(status == 0){
